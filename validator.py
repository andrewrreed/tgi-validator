    ...
    try:
        ...
        while True:
            if time.time() - start_time > timeout:
                raise TimeoutError("Container startup timed out after 5 minutes")
        ...
        if process and process.pid:
            ...
            try:
                ...
                container_id = subprocess.check_output(get_container_cmd, shell=True).decode().strip()
                ...
                if container_id:
                    ...
                    if not subprocess.check_output(check_cmd, shell=True).decode().strip():
...
def try_run_model_with_shards(model_id: str, max_total_tokens: int, initial_shards: int, min_shards_by_tokens: dict, gpu_type: str, image_uri: str) -> tuple[bool, dict]:
    ...
    try:
        ...
        if start_index == 0 and shard_options[0] < start_shards:
            logger.error(f"Model requires {start_shards} shards, but maximum available is {shard_options[-1]}")
            return False, {"error": "Insufficient GPU memory"}
    
    logger.info(f"Starting with {shard_options[start_index]} shard(s) based on VRAM and previous results")
    
    for num_shards in shard_options[start_index:]:
        total_vram = num_shards * gpu_vram
        if total_vram < model_vram:
            logger.warning(f"Skipping {num_shards} shard(s) as total VRAM ({total_vram}GB) < model size ({model_vram}GB)")
            continue
...
def main():
    ...
    if args.gpu_type not in [gpu["gpu_type"] for gpu in GPU_TYPES]:
        parser.error(f"Invalid GPU type. Must be one of: {', '.join([gpu['gpu_type'] for gpu in GPU_TYPES])}")
